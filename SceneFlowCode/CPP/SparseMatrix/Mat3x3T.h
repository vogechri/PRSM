...
public:
  ...
  friend Vec3 operator*(const Vec3& v, Mat3x3T p )
  {
    return Vec3 ( p.elem_[0]|v, p.elem_[1]|v, p.elem_[2]|v );
  };
  ...
  bool invert()
  {
    ...
    if ( dt < std::numeric_limits<Scalar>::epsilon() ) return false;
...
private:
  ...
  Scalar under_det(unsigned int i, unsigned int j)
  {
    unsigned int i_1 = (i + 1) % 3;
    unsigned int i_2 = (i + 2) % 3;
    unsigned int j_1 = (j + 1) % 3;
    unsigned int j_2 = (j + 2) % 3;

    if (i_1 > i_2) 
    {
      unsigned int swap = i_2;
      i_2 = i_1;
      i_1 = swap;
    }
    if (j_1 > j_2) 
    {
      unsigned int swap = j_2;
      j_2 = j_1;
      j_1 = swap;
    }
