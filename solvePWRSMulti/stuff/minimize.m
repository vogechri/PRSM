...
if max(size(length)) == 4, MAX = length(4); SIG=length(3);RHO = SIG/2; red=length(2); length=length(1); 
elseif max(size(length)) == 3, SIG=length(3);RHO = SIG/2; red=length(2); length=length(1); 
elseif max(size(length)) == 2, red=length(2); length=length(1); 
else red=1;
end
...
% first derivative 2nd: numerical deriv
...
%        if isnan(f3) || isinf(f3) || any(isnan(df3)+isinf(df3)), error(''), end
  ...
  % SPENDS WAY TOO LONG IN THIS PART OF THE SCHEME: UPDATE IN THE ORDER OF
  % e-5 AND SO :) -> CRAP: |x3*s| >= eps to be tested here
  while (abs(d3) > -SIG*d0 || f3 > f0+x3*RHO*d0) && M > 0  % keep interpolating
    ...
    if ~isreal(x3) || isnan(x3) || isinf(x3)
      x3 = (x2+x4)/2;               % if we had a numerical problem then bisect
    end
    x3 = max(min(x3, x4-INT*(x4-x2)),x2+INT*(x4-x2));  % don't accept too close
%     if ~isreal(x3) || any(isnan(s)) || ~isreal(s)
%       a =1;
%     end
    [f3 df3] = feval(f, X+x3*s, varargin{:});
    if f3 < F0, X0 = X+x3*s; F0 = f3; dF0 = df3; end         % keep best values
    M = M - 1; i = i + (length<0);                             % count epochs?!
    d3 = df3'*s;                                                    % new slope: yields direction of the update
  end                                                       % end interpolation


%  if abs(d3) < -d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    X = X+x3*s; f0 = f3; fX = [fX' f0]';                     % update variables
%    fprintf('%s %6i;  Value %4.6e\r', S, i, f0);
    s = ((df3-df0)'*df3)/(df0'*df0)*s - df3;   % Polack-Ribiere CG direction
