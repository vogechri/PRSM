function [cam, ref, imageName, flow2DGt, flow2DGt_noc] = loadZealandFlowCentered( strFolder, nr, p, iName )
...
framefac = 2;% not only a single but 2 frames

strNumber0 = sprintf( '%04d', nr );
strNumber1 = sprintf( '%04d',nr+1*framefac );
imageName = sprintf('%s_%s', iName, strNumber0);
...
ImgC{1} = double(sub_imread(sprintf('%s/left_%s/img_%s_c0.pgm', strFolder, iName, strNumber0 )))/65535;
ImgC{2} = double(sub_imread(sprintf('%s/left_%s/img_%s_c0.pgm', strFolder, iName, strNumber1 )))/65535;
...
ImgL{1} = double(sub_imread(sprintf('%s/center_%s/img_%s_c1.pgm', strFolder, iName, strNumber0 )))/65535;
ImgL{2} = double(sub_imread(sprintf('%s/center_%s/img_%s_c1.pgm', strFolder, iName, strNumber1 )))/65535;


if nr > 1
  strNumber2 = sprintf( '%04d',nr-1*framefac );
  ImgCold = double(sub_imread(sprintf('%s/left_%s/img_%s_c0.pgm', strFolder, iName, strNumber2 )))/65535;
  ImgRold = double(sub_imread(sprintf('%s/right_%s/img_%s_c2.pgm', strFolder, iName, strNumber2 )))/65535;
  ImgLold = double(sub_imread(sprintf('%s/center_%s/img_%s_c1.pgm', strFolder, iName, strNumber2 )))/65535;
end

strNumber3 = sprintf( '%04d',nr+2*framefac );
if exist( sprintf('%s/left_%s/img_%s_c0.pgm', strFolder, iName, strNumber3), 'file')
  ImgCnew = double(sub_imread(sprintf('%s/left_%s/img_%s_c0.pgm', strFolder, iName, strNumber3 )))/65535;
  ImgRnew = double(sub_imread(sprintf('%s/right_%s/img_%s_c2.pgm', strFolder, iName, strNumber3 )))/65535;
  ImgLnew = double(sub_imread(sprintf('%s/center_%s/img_%s_c1.pgm', strFolder, iName, strNumber3 )))/65535;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sx       = 1031.02;
sy       = 0.998045*1031.02;
...
b1  =   0.299663; % center - right
b2  = - 0.505707; % lc left:center
