function plotDataE( img1, img2, dm1X, dm2X, fNr, par, fname )

minD=-1;maxD=5;
f=figure(fNr+1000);set(f, 'visible','off');
imshow(dm1X, [minD,maxD], 'Border','tight'), colormap(hsv),axis image,axis off;
%tmp1 = getframe(f);tmp1=tmp1.cdata;
tmp1 = export_fig(f, '-nocrop', '-a1');
close(f);
f=figure(fNr+1000);set(f, 'visible','off');
imshow(dm2X, [minD,maxD], 'Border','tight'), colormap(hsv),axis image,axis off;
...
truesize(fNr);
export_fig( sprintf('%s/Data_%d_%s.png', par.sFolder, par.imgNr, fname), '-m1');
close(f);
